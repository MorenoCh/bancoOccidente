...
import tn5250j.tools.filters.FileFieldDef;
import tn5250j.tools.filters.OutputFilterInterface;
...
public class FTP5250Prot {
   ...
   public synchronized void addFTPStatusListener(FTPStatusListener listener) {
      ...
      if (listeners == null) {
         listeners = new Vector<FTPStatusListener>(3);
      }
    ...
    */
   public boolean login(String nombreUsu, String miCla) {
      ...
      //OAC - No puede ser comentariado, es el modelo de autenticacion de
      // usuario, se cambia nombre de variable
      executeCommand("USER", nombreUsu);

      // send password to server
      //OAC - No puede ser comentariado, se envia el pass de conexion
      //se cambia nombre de variable
      int resp = executeCommand("PASS", miCla);
...
//                  parseResponse();
   ...
   private int executeCommand(String cmd, String params) {
      ...
      if(params != null)
         //OAC - Se cambia la linea para no incluir los parametros
         //ftpOutputStream.print(cmd + " " + params + "\r\n");
         ftpOutputStream.print("Los parametros son nulos " + "\r\n");
      else
